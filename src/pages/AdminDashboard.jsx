...
const [applicants, setApplicants] = useState(() => {
  ...
  // --- RE-ENGINEERED DYNAMIC SYNCHRONIZATION ---
  useEffect(() => {
    // 1. Fetch live system users from shared pipeline storage
    let savedUsers = [];
    try {
    ...
    // 2. Fetch live job cards generated by the main portal forms
    let savedJobs = [];
    try {
    ...
    // 3. Fetch live applications submitted by candidates across views
    let savedApps = [];
    try {
  ...
  // --- DYNAMIC SEARCH & FILTER PARSING MATRIX ---
  ...
  const COLORS = ['#0d9488', '#2e6da4', '#27ae60', '#e67e22', '#9b59b6'];
    ...
    <div style={styles.container}>
      ...
      <div style={styles.mainContent}>
        ...
        <section style={styles.workspaceSection}>
          <h2 style={styles.sectionTitle}>💼 Live Connected Workspace — Job Cards ({filteredJobs.length})</h2>
          <div style={styles.cardsGridContainer}>
            {filteredJobs.length === 0 ? (
              <p style={styles.emptyMessageText}>No active job postings loaded from application database.</p>
            ) : (
                ...
                return (
                  <div key={job.id} style={styles.employerJobCard}>
                    ...
                    <div style={styles.applicantsContainerSection}>
                      ...
                      {linkedApps.length === 0 ? (
                        <p style={styles.noApplicantsText}>No matching candidate forms received.</p>
                      ) : (
        ...
        <section style={styles.workspaceSection}>
          <h2 style={styles.sectionTitle}>📊 Live Admin Analytics Engines</h2>
          <div style={styles.kpiGridContainer}>
            <div style={{ ...styles.kpiCard, backgroundColor: '#2e6da4' }}>
              <div><div style={styles.kpiLabel}>Total Jobs</div><div style={styles.kpiValue}>{jobs.length}</div></div>
            ...
            <div style={{ ...styles.kpiCard, backgroundColor: '#0d9488' }}>
                ...
                <tbody>
                  {filteredJobs.map((job) => (
                    <tr key={job.id} style={styles.tableDataRow}>
                      ...
                      <td style={styles.tableCell}><span style={styles.ownerTag}>{job.postedBy || 'System Portal'}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const styles = {
  ...
  miniUserAvatar: { width: '24px', height: '24px', borderRadius: '50%', backgroundColor: '#0d9488', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.75rem' },
